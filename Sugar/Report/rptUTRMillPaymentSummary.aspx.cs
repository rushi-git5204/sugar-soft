...
public partial class Report_rptUTRMillPaymentSummary : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("CryMillPaymentSummary.rpt"));
                rpt.SetDataSource(dt);

                CryMillPaymentSummary.ReportSource = rpt;
                ...
                CryMillPaymentSummary.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (ac_code != "0")
            {
                //SqlCommand cmd = new SqlCommand("select * from qrymillpaymentbalance where  payableamount!=0 and Tender_Date between '" + from +
                //        "' and '" + to + "' and  Payment_To='" + ac_code + "' and Year_Code='"
                //        + Convert.ToInt32(Session["year"].ToString()) + "' and Company_Code='"
                //+ Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);

                SqlCommand cmd = new SqlCommand("select * from qrymillpaymentbalance where  payableamount!=0 and Tender_Date between '" + from +
                       "' and '" + to + "' and  Payment_To='" + ac_code + "' and   Company_Code='"
               + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.CryMillPaymentSummary.ReportSource = null;

        CryMillPaymentSummary.Dispose();
