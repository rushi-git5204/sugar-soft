    ...
    private void BindList()
    {
        try
        {
            ...
            if (Broker_Code != string.Empty)
            {
                ...
                //    " (ISNULL((Select SUM(t.amount) as Recieved from nt_1_transactdetail t where t.Tran_Type IN('BR','CR') and t.Tender_No = u.doc_no and t.tenderdetailid = u.saleid and t.Company_Code=u.Company_Code and t.Year_Code=u.Year_Code),0)+ISNULL((Select SUM(pr.Bill_Amount) as Reci" +
                //    " from qrysugarpurchasereturnhead pr WHERE pr.PURCNO=u.doc_no and pr.Company_Code=u.Company_Code and pr.Year_Code=u.Year_Code),0)) as Recieved," +
                //    " (u.Bill_Amount-ISNULL((Select SUM(t.amount) as Recieved from nt_1_transactdetail t where t.Tran_Type IN('BR','CR') and t.Tender_No = u.doc_no and t.tenderdetailid = u.saleid and t.Company_Code=u.Company_Code and t.Year_Code=u.Year_Code),0) - ISNULL((Select SUM(pr.Bill_Amount) as Reci" +
                //    " from qrysugarpurchasereturnhead pr WHERE pr.PURCNO=u.doc_no and pr.Company_Code=u.Company_Code and pr.Year_Code=u.Year_Code),0)) as Short,u.BROKER,u.ASN from " + tblPrefix + "qryVoucherSaleUnion1 u" +
                //    " left outer join " + tblPrefix + "AccountMaster a on u.mill_code=a.Ac_Code and u.Company_Code=a.Company_Code where u.Ac_Code=" + Broker_Code + " and u.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and u.Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and u.doc_date between '" + from + "' and '" + to + "'";
            ...
            else
            {
                ...
                //    " (u.Bill_Amount-ISNULL((Select SUM(t.amount) as Recieved from nt_1_transactdetail t where t.Tran_Type IN('BR','CR') and t.Tender_No = u.doc_no and t.tenderdetailid = u.saleid and t.Company_Code=u.Company_Code and t.Year_Code=u.Year_Code),0) - ISNULL((Select SUM(pr.Bill_Amount) as Reci" +
                //    " from qrysugarpurchasereturnhead pr WHERE pr.PURCNO=u.doc_no and pr.Company_Code=u.Company_Code and pr.Year_Code=u.Year_Code),0)) as Short,u.BROKER,u.ASN from " + tblPrefix + "qryVoucherSaleUnion1 u" +
                //    " left outer join " + tblPrefix + "AccountMaster a on u.mill_code=a.Ac_Code and u.Company_Code=a.Company_Code where u.Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and u.Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " and u.doc_date between '" + from + "' and '" + to + "'";
