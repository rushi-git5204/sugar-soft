...
public partial class Sugar_Report_rptDeliveryOrderForGST : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //   rpt.SetDataSource(dt);
             ...
             tenderDate = clsCommon.getString("select Tender_DateConverted from qrytenderhead where Tender_No='" + tenderno +
    ...
    protected void btnMail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtEmail.Text != string.Empty && txtEmail.Text != "0")
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), " ", "alert('Mail Send successfully')", true);
                string qry = "update NT_1_deliveryorder set MailSend='Y' where company_code=" + Convert.ToInt32(Session["Company_Code"].ToString())
                              + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + "  and doid='" + doc_no + "'";
                ds = clsDAL.SimpleQuery(qry);
            }
            else
            {
                //Response.Write("<script>alert('Enter Email Id');</script>");
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), " ", "alert('Enter Email Id')", true);
            }
            Response.Write("<script>alert('Mail Send successfully');</script>");
        }
    ...
    protected void btnWhatsApp_Click(object sender, EventArgs e)
    {
        ...
        //}
        //string uploadedFileName = "DO.pdf";
        ...
        //string Url = "https://wawatext.com/api/send.php?number=91" + mobileNumber + "&type=text&message=" + message + "&instance_id=" + instanceid + "&access_token=" + accesstoken + "";
        ...
        //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "api-call", "javascript:sendPdfToWatsapp('" + bill_no + "','" + base64String + "', '" + uploadedFileName + "','" + instanceid + "','" + accesstoken + "','DO from new updated whatsapp api','" + mobileNumber + "', 'LATASOFTWARE');", true);
        ...
        if (table.Rows.Count > 0)
        {
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                string message = " HI " + Environment.NewLine + " DELIVERY ORDER FROM NAVKAR TRADERS PUNE"  + Environment.NewLine +
                      " DATE: " + dodocdate + "  " + Environment.NewLine + " MILL NAME: " + millshortname + "" + Environment.NewLine +
                      " DO NO: " + DONo + "  " + Environment.NewLine + " TRUCK NO: " + Lorryno + "  " + Environment.NewLine +
                      " QTY: " + Qty + "  " + Environment.NewLine + " GRADE: " + grade + "  " + Environment.NewLine + " MILL RATE: " + millrate + " " + Environment.NewLine +
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "api-call", "javascript:sendPdfToWatsapp('" + bill_no + "','" + base64String + "', '" + uploadedFileName + "','" + instanceid + "','" + accesstoken
                + "','DELIVER ORDER FROM NAVKAR TRADERS        DO NO: " + DONo + " TRUCK NO: " + Lorryno + "','" + Moblie_Number + "', 'LATASOFTWARE','" + txtWhatsapp.Text + "');", true);
