...
public partial class Report_rptProfitLossSP : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            frmdt = DateTime.Parse(clsGV.Start_Date, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("yyyy/MM/dd");
    ...
    //protected void lnkDO_Click1(object sender, EventArgs e)
    //{
    //    try
    //    {
    //        LinkButton lnkOV = (LinkButton)sender;
    //        DataListItem item = (DataListItem)lnkOV.NamingContainer;
    //        Label lblACAmountL = (Label)item.FindControl("lblACAmountL");
    //        string fromdt1 = clsGV.Start_Date;
    //        string todt1 = clsGV.To_date;
    //        string accode = lblACAmountL.Text;
    //        // Session["VOUC_NO"] = no;
    //        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ky", "javascript:sp('" + accode + "','" + fromdt1 + "','" + todt1 + "','DrCr')", true);
    //        lnkOV.Focus();
    //    }
    ...
    private void bindData()
    {
        try
        {
            ...
            qry = "select Group_Code,BSGroupName,group_Summary,group_Order,AC_CODE,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance   from "
                ...
                + "' and DOC_DATE>='" + frmdt + "' group by Group_Code,BSGroupName,group_Summary,group_Order,AC_CODE order by Group_Code,group_Order";
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                          ...
                          //  groupcode = Convert.ToInt32(dt.Rows[i]["Group_Code"].ToString());
                            if (i  >= totalrows )
                            {
                                groupcode = 556565;
                            }
    ...
    private void bindData_p()
    {
        try
        {
            ...
            qry = "select Group_Code,BSGroupName,group_Summary,group_Order,Ac_Code,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance   from "
                ...
                + ViewState["BalDate"].ToString() + "'  and DOC_DATE>='" + frmdt + "' group by Group_Code,BSGroupName,group_Summary,group_Order,Ac_Code order by Group_Code,group_Order";
            ds = clsDAL.SimpleQuery(qry);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
