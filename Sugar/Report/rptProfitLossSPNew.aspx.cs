...
public partial class Sugar_Report_rptProfitLossSPNew : System.Web.UI.Page
{
    ...
    {
        frmdt = Request.QueryString["fdt"];
        todt = Request.QueryString["tdt"];
        if (!Page.IsPostBack)
        {
            ...
            frmdt = DateTime.Parse(frmdt, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("yyyy/MM/dd");
            ...
            lblUpto.Text = "Profit & Loss upto " + DateTime.Parse(ViewState["BalDate"].ToString(), System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("dd/MM/yyyy");
    ...
    private void bindData()
    {
        try
        {
            ...
            int groupcode = 0;
            string groupname = "";
            double groupamountcr = 0.00;
            double groupamountdr = 0.00;
            string groupsummary = "";
            int totalrows = 0;
            double bal = 0.00;

            string qry = "";
            //qry = "select Group_Code,BSGroupName,group_Summary,group_Order,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance   from "
            //    + qryCommon + " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and group_Type='T' and DOC_DATE<='" + ViewState["BalDate"].ToString()
            //    + "' and DOC_DATE>='" + frmdt + "' group by Group_Code,BSGroupName,group_Summary,group_Order order by group_Order";
            qry = "select Group_Code,BSGroupName,group_Summary,group_Order,AC_CODE,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance   from "
              + qryCommon + " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and group_Type='T' and DOC_DATE between '" + frmdt
              + "' and '" + todt + "' group by Group_Code,BSGroupName,group_Summary,group_Order,Ac_Code order by Group_Code,group_Order";
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        groupcode = Convert.ToInt32(dt.Rows[0]["Group_Code"].ToString());
                        totalrows = dt.Rows.Count - 1;
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            //if (dt.Rows[i]["group_Summary"].ToString() == "Y")
                            //{    
                            int gpcode = Convert.ToInt32(dt.Rows[i]["Group_Code"].ToString());
                            groupname = dt.Rows[i]["BSGroupName"].ToString();
                            groupsummary = dt.Rows[i]["group_Summary"].ToString();       
                            bal = Convert.ToDouble(dt.Rows[i]["Balance"].ToString()) + 0.00;
                            if (bal > 0)
                            {
                                groupamountdr = groupamountdr + bal;
                                netDebit = netDebit + bal;

                            }
                            else
                            {
                                groupamountcr = groupamountcr + (bal * (-1));
                                netCredit = netCredit + (bal * (-1));
                            }
                            if (i >= totalrows)
                            {
                                groupcode = 556565;
                            ...
                            else
                            {
                                ...
                                groupcode = Convert.ToInt32(dt.Rows[i + 1]["Group_Code"].ToString());
                            }
                            if (groupcode != gpcode)
                            {
                                if (groupamountdr > 0)
                                {
                                    ...
                                    dr["Group_Code"] = gpcode;
                                    dr["groupname"] = groupname;
                                    dr["groupamount"] = groupamountdr;
                        ...
                        if (netCredit - netDebit > 0)
                        {
                            ...
                            netProfit = Math.Round((netCredit - netDebit), 2);
    ...
    protected void dtl_TradingDebit_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            else    //summary no show detail part
            {
                ...
                "select AC_CODE, Ac_Name_E,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance " +
                " from " + qryCommon + " " +
                " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and DOC_DATE between '" + frmdt
              + "' and '" + todt + "' and Group_Code=" + lblGroupCodeL.Text + " " +
                " group by AC_CODE,Ac_Name_E having SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) !=0";
                ds = clsDAL.SimpleQuery(qry);
                if (ds != null)
                {
    ...
    protected void dtl_TradingCredit_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            dtRInner.Columns.Add("AC_CODE", typeof(double));
            ...
            else    //summary no show detail part
            {
                ...
                " from " + qryCommon + " " +
                " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and DOC_DATE between '" + frmdt
              + "' and '" + todt + "' and Group_Code=" + lblGroupCodeR.Text + " " +
                " group by AC_CODE,Ac_Name_E having SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) !=0";
                ds = clsDAL.SimpleQuery(qry);
                if (ds != null)
                {
    ...
    private void bindData_p()
    {
        try
        {
            ...
            string qry = "";
            qry = "select Group_Code,BSGroupName,group_Summary,group_Order,Ac_Code,SUM(case drcr when 'D' then AMOUNT when 'C' then -amount end) as Balance   from "
                + qryCommon + " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and group_Type='P' and DOC_DATE between '" + frmdt
              + "' and '" + todt + "' group by Group_Code,BSGroupName,group_Summary,group_Order,Ac_Code order by Group_Code,group_Order";
            ds = clsDAL.SimpleQuery(qry);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            {
                                groupamountdr = groupamountdr + bal;
                                p_netDebit = p_netDebit + bal;

                            }
                            else
                            {
                                groupamountcr = groupamountcr + (bal * (-1));
                                p_netCredit = p_netCredit + (bal * (-1));
                            }
                            ...
                            if (groupcode != gpcode)
                            {
                                if (groupamountdr > 0)
                                {
                                    ...
                                    groupamountdr += bal;
                                    lblNetCredit1.Text = Convert.ToString(groupamountdr);
                                }
                                if (groupamountcr > 0)
                                {
                                    DataRow dr = dtLeft.NewRow();
                                    ...
                                    groupamountcr += -bal + p_netProfit;
                                    lblNetDebit1.Text = Convert.ToString(groupamountcr);
                                }
                                groupamountcr = 0;
                                groupamountdr = 0;
