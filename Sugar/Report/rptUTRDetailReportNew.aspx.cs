...
public partial class Report_rptUTRDetailReportNew : System.Web.UI.Page
{
    int billno;
    int company_code;
    int year_code;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isAuthenticate == "1" || User_Type == "A")
            {
                ...
                rpt.Load(Server.MapPath("cryUTRDetailReport.rpt"));
                rpt.SetDataSource(dt);

                cryUTRDetailReport.ReportSource = rpt;
                ...
                cryUTRDetailReport.RefreshReport();
    ...
    private DataTable GetData(string from, string to)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (utr_no != "0")
            {
                //SqlCommand cmd = new SqlCommand("select * from qryutrdobalancemain where  doc_date between '" + FromDt +
                //    "' and '" + ToDt + "'  and utr_no='" + utr_no + "' and Year_Code='"
                //    + Convert.ToInt32(Session["year"].ToString()) + "' and Company_Code='"
                //    + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);

                SqlCommand cmd = new SqlCommand("select * from qryutrdobalancemain where  doc_date between '" + FromDt +
                   "' and '" + ToDt + "'  and utr_no='" + utr_no + "' and  Company_Code='"
                   + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
            ...
            else if (ac_code != "0")
            {
                ...
                SqlCommand cmd = new SqlCommand("select * from qryutrdobalancemain where  doc_date between '" + FromDt +
                  "' and '" + ToDt + "' and mill_code='" + ac_code +  "'and Company_Code='"
                   + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
            ...
            {
                //SqlCommand cmd = new SqlCommand("select * from qryutrdobalancemain where  doc_date between '" + from +
                //    "' and '" + ToDt + "'  and utr_no='" + utr_no + "' and mill_code='" + ac_code + "' and Year_Code='"
                //    + Convert.ToInt32(Session["year"].ToString()) + "' and Company_Code='"
                //    + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);

                SqlCommand cmd = new SqlCommand("select * from qryutrdobalancemain where  doc_date between '" + from +
                    "' and '" + ToDt + "'  and utr_no='" + utr_no + "' and mill_code='" + ac_code + "' and Company_Code='"
                    + Convert.ToInt32(Session["Company_Code"].ToString()) + "'", con);
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        //rprt1.Dispose();
        //GC.Collect();
        this.cryUTRDetailReport.ReportSource = null;

        cryUTRDetailReport.Dispose();
