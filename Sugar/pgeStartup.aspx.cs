...
public partial class pgeStartup : System.Web.UI.Page
{
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        Session["EmailPassword"] = enc.Decrypt(dt.Rows[0]["EmailPassword"].ToString());
                        ...
                        if (ds != null)
                        {
                            if (ds.Tables.Count > 0)
                            {
                                ...
                                if (dt.Rows.Count > 0)
                                {
                                    ...
                                    Session["DepreciationAC"] = dt.Rows[0]["SaleTCSAc"].ToString();
                        ...
                        #endregion
                        #region whatsappapi
                        qry = "select Instance_Id,Access_token from tblWhatsAppURL where Company_Code=" + Session["Company_Code"].ToString();
                        ds = clsDAL.SimpleQuery(qry);
                        if (ds != null)
                        {
                            if (ds.Tables.Count > 0)
    ...
    private void UserSecurity()
    {
        try
        {
            string qry = "select * from UserSecurity where Company_Code='" + Session["Company_Code"].ToString() + "'";
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            ds = clsDAL.SimpleQuery(qry);
            if (ds != null)
